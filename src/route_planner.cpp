...
RouteModel::Node *RoutePlanner::NextNode() {
  ...
  int min = open_list[0]->g_value + open_list[0]->h_value;
  for(auto it =open_list.begin() ; it!=open_list.end() ; it++)
  {
    int f = (*it)->h_value + (*it)->g_value;
    if(f<min){min = f; min_it = it;}
  }
  open_list.erase(min_it);
  return open_list[min_it-open_list.begin()];
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
  ...
  do{
  auto it = path_found.insert(path_found.begin(),*current_node);
